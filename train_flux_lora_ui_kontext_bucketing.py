...
from utils.image_utils_flux import load_image, compute_text_embeddings, replace_non_utf8_characters, create_empty_embedding, get_empty_embedding, cache_file, cache_multiple, crop_image,get_md5_by_path,vae_encode,read_image, get_nearest_resolution
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--enable_bucketing",
        action="store_true",
        help="Enable bucketing for training with multiple aspect ratios.",
    )
                    ...
                    for embedding_object_key in tqdm(embedding_objects.keys()):
                        ...
                        for image_config_key in image_configs.keys():
                            ...
                            if args.enable_bucketing:
                                image_for_size = read_image(image_path)
                                _, bucket_resolution = get_nearest_resolution(image_for_size, resolution=resolution)
                                target_width, target_height = bucket_resolution
                                image = cv2.resize(image_for_size, (target_width, target_height), interpolation=cv2.INTER_AREA)
                            else:
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(transformer):
                ...
                if global_step % 100 == 0:
                    print(f"[Step {global_step}] 清理显存缓存中...")
                    release_memory()
